...
module namespace plugin = "http://marklogic.com/data-hub/plugins/Bxx";
...
(:
  Attribute b is stereotyped in the model as follows:: 
    resolvedType: 
      string
:)
   let $_ := map:put($model, "b", "Bxxb") (: type: string, req'd: true, array: false :)
...
(:
  Attribute id is stereotyped in the model as follows:: 
    resolvedType: 
      string
:)
   let $_ := map:put($model, "id", "Bxx"  || $sampleData) (: type: string, req'd: true, array: false :)
