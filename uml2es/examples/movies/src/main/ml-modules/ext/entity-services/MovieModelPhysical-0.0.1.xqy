...
) as map:map
{
    ...
    let $parentalCerts  := json:to-array(
        for $cert in $source-node/parentalCerts return
            movieModelPhysical:extract-instance-ParentalCertificate($cert, $movieId)
    )
    ...
    then $instance
        ...
        => map:with("parentalCerts", $parentalCerts)
};
...
declare function movieModelPhysical:extract-instance-ParentalCertificate(
    $source as item()?, $movieId as xs:string
) as map:map
{

    (: IMPLNOTE - It takes movieId as param now, and that gets added to the doc :)
